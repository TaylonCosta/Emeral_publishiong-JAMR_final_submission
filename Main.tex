...
\begin{figure}[htbp]
    ...
    \caption{\textcolor{red}{\textbf{Gantt chart illustrating the integrated production and distribution schedule}. The timeline is vertically partitioned into two operational stages: Production Scheduling involving parallel machines and Delivery Routes utilizing a vehicle fleet. The visualizationemonstrates the synchronization constraints, where the departure time of each vehicleis strictly determined by the completion time of the last manufactured job assigned to that route.} (Source: Authors' own work)}
...
\begin{figure}[htbp]
    ...
    \caption{\textcolor{red}{\textbf{Operational flowchart of the proposed PPO-VND framework}. Diagram depicting the hybrid metaheuristic architecture in which the PPO agent serves as the high-level controller, selecting a NSH at each iteration using action masking to ensure valid choices.} (Source: Authors' own work)}
...
\textcolor{red}{\subsection{PPO Training}}
\label{subsec:PPOtraining}

\textcolor{red}{As mentioned in earlier sections, we optimized the hyperparameters of the PPO agent using OptunaAt each trial, Optuna samples a complete PPO configuration by drawing from a mixture of categorical and continuous distributions. The categorical parameters include: batch size (8-512), number of rollout steps (8-2048), discount factor (0.--0.9999), clipping range (0.1-0.4), number of training epochs per update (1-20), GAE parameter $\lambda$ (0.8-1.0), maximum gradient norm (0.3-5), network architecture, and activation function (tanh or ReLU). Continuous parameters include the learning rate (log-uniform in [1e-5, 1]) and the entropy coefficient (log-uniform in [1e-8, 1e-1]), along with a uniform value-function loss coefficient in [0, 1]. To ensure valid training configurations, the batch size is constrained not to exceed the number of rollout steps, and the selected architecture is mapped to predefined multilayer perceptrons for the policy and value networks (either [64, 64] or [256, 256] units per layer). The hyperparameters found via Optuna utilized in training are shown in Table \ref{tab:hyperparams}.}


\begin{table}
\caption{\textcolor{red}{Hyperparameters utilized on training found via Optuna tunnning.} \label{tab:hyperparams}}
\centering % Este comando centraliza a tabela.
{
\centering % Este comando centraliza a tabela.
% \resizebox{\textwidth}{!}{%
\begin{tabular}{|l|c|}
\hline
\textcolor{red}{Parameter} & \textcolor{red}{Value} \\
\hline
\textcolor{red}{Learning rate decay2 }   &\textcolor{red}{0.001} \\
\textcolor{red}{Gamma decay             }   &\textcolor{red}{ 0.98  } \\
\textcolor{red}{Mini batch size         }   &\textcolor{red}{ 256  } \\
\textcolor{red}{Clipping range          }   &\textcolor{red}{ 0.2 }  \\
\textcolor{red}{Entropy coefficient     }   &\textcolor{red}{ 0.0}   \\
\textcolor{red}{Number of envs          }   &\textcolor{red}{ 4 }  \\
\textcolor{red}{Number of timesteps     }   &\textcolor{red}{ 1000}   \\
\textcolor{red}{Number of steps         }   &\textcolor{red}{ 100}   \\
\textcolor{red}{Number of steps         }   &\textcolor{red}{ 20}   \\
\textcolor{red}{Policy                  }   &\textcolor{red}{ MlpPolicy}   \\
\textcolor{red}{lambda                  }   &\textcolor{red}{0.8} \\ 
\hline
\end{tabular}}
%}
\\
...
Table \ref{tab:ResultadosNSH} provides the average GAPs for the eight Neighborhood Search Heuristics. Meanwhile, Table \ref{tab:Resultados} displays the average GAPs for the other algorithms discussed in this work. Each row in these tables presents the average GAPs across 60 instances, categorized by the number of jobs and machines. The smallest average GAPs are highlighted in bold. The final rows of each table summarizes the average\textcolor{red}{ and standard deviation} of GAPs for the entire set of instances.

\begin{table}
\centering % Este comando centraliza a tabela.
 \caption{\textcolor{red}{\textbf{Performance comparison of NSH based on average optimality gap (\%)}. The results demonstrate the superior efficiency of NSH 6 compared with simlar NSH's.} \label{tab:ResultadosNSH}}
 {
 \centering % Este comando centraliza a tabela.
% \resizebox{\textwidth}{!}{%
...
\textcolor{red}{Std. deviation   }   &\textcolor{red}{ 15.49\%  }   &\textcolor{red}{ 18.83\%  }   &\textcolor{red}{ 14.34\%  }   &\textcolor{red}{ 16.01\%  }   &\textcolor{red}{ 15.98\%  }   &\textcolor{red}{ 7.83 }   &\textcolor{red}{ 14.9  }   &\textcolor{red}{ 13.39 } \\ \hline
\end{tabular}}
%}
{}
\text{(Source: Authors own work)}
...
\caption{\textcolor{red}{\textbf{Comparative analysis of optimality gaps across varying instance dimensions}. Average, Best, and Worst solution gaps for the exact method (MILP) versus heuristic approaches.}\label{tab:Resultados}}
\centering % Este comando centraliza a tabela
% \resizebox{\textwidth}{!}{%
\resizebox{\textwidth}{!}{
...
\textcolor{red}{Std. deviation }   &\textcolor{red}{ 34.78\%}   &\textcolor{red}{ 8.52\% }   &\textcolor{red}{ 19.28\%}   &\textcolor{red}{ 8.19\%}   &\textcolor{red}{ 7.83\%  }   &\textcolor{red}{ 11.28\% }   &\textcolor{red}{ 19.7\%}   &\textcolor{red}{ 8.05\% }   &\textcolor{red}{ 18.33\%}\\ \hline
\end{tabular}
%}
}{}
\text{(Source: Authors own work)}
...
\begin{table}
\centering % Este comando centraliza a tabela.
\caption{\textcolor{red}{\textbf{Comparison of average computational processing time (in seconds)}. The data highlights the runtime disparity between exact and heuristic approaches.}\label{tab:tempo}}
 {
 \resizebox{\textwidth}{!}{%
...
\textcolor{red}{Std. deviation    }   &\textcolor{red}{ 27.80 }   &\textcolor{red}{ 99.60   }   &\textcolor{red}{ 188.41 }   &\textcolor{red}{ 7.85     }   &\textcolor{red}{ 6.41  }   &\textcolor{red}{ 9.97  }   &\textcolor{red}{ 12.62  }   &\textcolor{red}{ 48.80  }   &\textcolor{red}{ 6.78  }   &\textcolor{red}{ 10.99  }   &\textcolor{red}{ 12.13  }   &\textcolor{red}{ 36.06} \\ \hline
\end{tabular}}
}
{}
\text{(Source: Authors own work)}
...
A holistic evaluation, considering overall GAP outcomes, decisively positions PPO-VND as the top-performing methodology, boasting an exceptional average GAP of 29\% \textcolor{red}{and standard deviation of 8.52\%}. The framework follows, achieving a GAP of 36\%. NSH 6 ranks third with a GAP of 37\%. The remaining algorithms are classified as follows: MILP (40\%), RVND (41\%), NSH 8 (53\%), NSH 2 (64\%), NSH 5 (65\%), NSH 7 (69\%), NSH 4 (70\%), NSH 1 (71\%), and finally, NSH 3 (74\%).
...
\begin{table}
\centering
\caption{\textcolor{red}{Assessment of solution quality based on Relative Percentage Improvement (RPI). Quantification of the improvement of each method relative to the initial solution.} \label{tab:ResultadosRPI}}
...
\textcolor{red}{Std. deviation }   &\textcolor{red}{ 11.5\%     }   &\textcolor{red}{ 7.7\%     }   &\textcolor{red}{ 12.8\%  }   &\textcolor{red}{ 8.4\%  }   &\textcolor{red}{ 4.3\% }   &\textcolor{red}{ 7.3\% }   &\textcolor{red}{ 6.2\%  }   &\textcolor{red}{ 8.9\%  }   &\textcolor{red}{ 12.5\%  }   &\textcolor{red}{ 8.7\%  }   &\textcolor{red}{ 10.4\%} \\ \hline
\end{tabular}}
{}
\text{(Source: Authors own work)}
\end{table}
...
\textcolor{red}{\subsection{Practical Case}}
\textcolor{red}{An practical case is considered based on a regional manufacturing and distribution system operating under make-to-order conditions. The system consists of multiple unrelated parallel machines with machine-dependent processing and setup times, responsible for producing customized orders. Once production is completed, the orders must be delivered to geographically dispersed customers using a limited-capacity vehicle fleet departing from a central depot.}

\textcolor{red}{In this context, production scheduling decisions directly affect vehicle departure times, while routing decisions influence delivery completion and customer tardiness. The proposed framework is applied to jointly determine machine assignments, job sequences, and delivery routes, aiming to minimize total weighted tardiness. This illustrative case reflects typical constraints found in real logistics operations, such as heterogeneous processing capabilities, tight delivery deadlines, and limited transportation resources, and serves to demonstrate the practical applicability of the integrated optimization approach.}
